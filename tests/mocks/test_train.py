...
async def test_send_next_train():
  ...
  result = await send_next_train(mock_func)
...
async def test_send_train_info():
  ...
  result = await send_train_info(mock_func)
  assert is_successful(result)
  assert len(called_with) == 1
...
async def test_get_registered_lines():
  ...
  登録済みの路線取得をテストします。
  """
  result = await get_registered_lines()
  assert is_successful(result)
  lines = result.unwrap()
...
async def test_limit_minute():
  ...
  """
  # 初期値の取得
  initial_result = await get_limit_minute()
  ...
  set_result = await set_limit_minute(new_minute_value)
  assert is_successful(set_result)

  # 新しい値を取得
  new_result = await get_limit_minute()
