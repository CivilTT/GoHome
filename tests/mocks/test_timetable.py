...
async def test_get_timetable():
  ...
  """
  # 既存の時刻表の取得をテスト
  timetable_result = await get_timetable(1)
  ...
  # 存在しない時刻表の取得をテスト
  timetable_result = await get_timetable(999)
  assert is_successful(timetable_result)
  timetable = timetable_result.unwrap()
  assert timetable is not None
  assert len(timetable.items) == 0
...
async def test_set_timetable():
  ...
  result = await set_timetable("New Line", "New Station", "New Bound", options)
  assert is_successful(result)
  new_line_id = result.unwrap()

  timetable_result = await get_timetable(new_line_id)
  assert is_successful(timetable_result)
  timetable = timetable_result.unwrap()
  assert timetable is not None
  assert len(timetable.items) > 0
...
async def test_remove_timetable():
  ...
  # まず、削除する時刻表を追加
  options = TimetableSetterOptions()
  add_result = await set_timetable("To Remove", "Station", "Bound", options)
  new_line_id = add_result.unwrap()

  # 存在することを確認
  assert is_successful(await get_timetable(new_line_id))

  # 次に、削除
  remove_result = await remove_timetable(new_line_id)
